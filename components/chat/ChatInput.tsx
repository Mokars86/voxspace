...
const ChatInput: React.FC<ChatInputProps> = ({ onSend, onTyping, replyTo, onCancelReply }) => {
    ...
    const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>, typeOverride?: 'image' | 'video' | 'file' | 'audio') => {
        if (e.target.files && e.target.files[0]) {
            ...
            onSend(file.name, type, file, undefined);
    ...
    const handleSendClick = () => {
        if (inputValue.trim()) {
            onSend(inputValue, 'text', undefined, undefined);
            setInputValue('');
        }
    };
                    ...
                    {isRecording ? (
                        <div className="flex-1 bg-red-50 dark:bg-red-900/10 rounded-3xl flex items-center justify-between px-4 py-3 animate-pulse border border-red-100 dark:border-red-900/30">
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <button onClick={() => stopRecording(true)} className="w-10 h-10 bg-primary text-primary-foreground rounded-full flex items-center justify-center shadow-lg active:scale-95 transition-transform">
